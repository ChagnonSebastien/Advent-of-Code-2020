...
try:
    ...
    for col in canvas.keys():
        if len(canvas.get(col)) > maxHeight:
            maxHeight = len(canvas.get(col))

    for i in range(maxHeight):
        row = ""
        for col in canvas.keys():
            color = 0 if canvas.get(col).get(i) is None else canvas.get(col).get(i)
            row = row + (" " if color == 0 else "#")
        print(row)
